...
class Resultats(commands.Cog):
    ...
    async def resultats(self, ctx, mention_gagnant: discord.User, item, prix, mention_vendeur: discord.User):
        em = discord.Embed(
            description=f"**GG** à {mention_gagnant.mention}, tu remportes {item} au prix de ***{prix}*** vendu par {mention_vendeur.mention}. Je vous invite à vous MP pour faire le trade.",
            color=0xFFA500)
