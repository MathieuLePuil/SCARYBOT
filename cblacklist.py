...
class Cblacklist(commands.Cog):
    ...
    async def cblacklist(self, ctx, blacklist_user : discord.User, duree):
        ...
        embed = discord.Embed(description = f"**{blacklist_user}** a été blacklist des **Commandes**! \n \n Tu pourras de nouveau commander dans {duree}.", color = 0xFFA500)
        embed.set_thumbnail(url = blacklist_user.avatar_url)

        embed2 = discord.Embed(description = f"**{blacklist_user}** a été blacklist des **Commandes**! \n \n Cette sanction est permanente.", color = 0xFFA500)
    ...
    async def cunblacklist(self, ctx, blacklist_user : discord.User):
        ...
        await blacklist_user.remove_roles(roleBarre1)
        await blacklist_user.remove_roles(roleBarre2)
        embed = discord.Embed(description = f"**{blacklist_user}** a été unblacklist des **Commandes**! \n \n Tu peux de nouveau commander des items.", color = 0xFFA500)
        embed.set_thumbnail(url = blacklist_user.avatar_url)
        await ctx.send(embed = embed)
        await log_channel.send(embed = embed) 
