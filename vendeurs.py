...
class Vendeurs(commands.Cog):
    ...
    async def rcv(self, ctx, status): 
        ...
        if status == "open":
            await ctx.send(embed = embedo, components=[Button(style=ButtonStyle.URL, label = "Formulaire Vendeur", url = "https://bit.ly/3gGqw0a", custom_id = "vendeur")])
